<?php
	...
	if($age == 0){
		$query = "SELECT DISTINCT colour FROM Students WHERE CURDATE() > arrival_date && CURDATE() < departure_date";
	}
	else if($age == 1){
		$query = "SELECT DISTINCT colour FROM Students WHERE CURDATE() > arrival_date && CURDATE() < departure_date && age <= 13";
	}
	else if($age == 2){
		$query = "SELECT DISTINCT colour FROM Students WHERE CURDATE() > arrival_date && CURDATE() < departure_date && age > 13";
	}
	...
	if($response) {
		...
		while($row = mysqli_fetch_array($response)) {
			echo 
				...
				<tr>
					<th colspan="4" style="text-align:center">' . $row['colour'] . '</th>
				</tr>
				<tr>
				...
				if($age == 0){
					$query2 = "SELECT name, english_name, student_id, room FROM Students WHERE colour = \"" . $row['colour']. "\" && 
